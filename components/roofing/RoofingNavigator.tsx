        ...
        {roofingServices.map((service, index) => (
          <li key={service.slug} className="border-b border-border-subtle">
            <button
              ...
              className={`group w-full flex items-center gap-5 px-3 py-5 text-left cursor-pointer transition-colors duration-150 ${
                index === activeIndex
            ...
            >
              ...
              <span className="font-heading font-semibold text-xl uppercase tracking-[0.01em]">
                {service.shortTitle}
              </span>
              <span
                className={`ml-auto w-2 h-2 shrink-0 transition-colors duration-150 ${
                  index === activeIndex
                    ? "bg-brand-primary"
      ...
      <div className="lg:hidden flex flex-col divide-y divide-border-subtle border-t border-b border-border-subtle">
        {roofingServices.map((service, index) => (
          <details key={service.slug} open={index === 0} className="group">
            <summary className="flex items-center justify-between gap-4 py-5 px-2 cursor-pointer list-none transition-colors duration-150 hover:bg-surface-subtle">
